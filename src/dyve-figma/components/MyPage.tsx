...
export default function MyPage({ navigate }: MyPageProps) {
  ...
  const [userType, setUserType] = useState<'user' | 'artist' | 'venue' | null>(null);
  ...
  const handleDevLogin = (userType: 'user' | 'artist' | 'venue') => {
    setIsLoggedIn(true);
    ...
    toast.success(messages[userType]);
  };
  ...
  return (
    <div className="min-h-screen pb-20 bg-black">
      ...
      <div className="px-6 py-8">
        ...
        {/* My Performances - Only visible for artist/venue users */}
        ...
        <div className="bg-[#1A1A1A] rounded-2xl p-6 border border-white/5">
          ...
          <div className="space-y-3 text-gray-500 text-sm">
            <button 
              ...
              className="w-full text-left hover:text-white transition font-medium"
            >
              알림 설정
            </button>
            <button 
        ...
        <div className="bg-gradient-to-br from-purple-900/20 to-blue-900/20 rounded-2xl p-6 border border-purple-500/20 mt-6">
          ...
          <div className="space-y-2">
            <button 
              ...
              className="w-full bg-gradient-to-r from-blue-600 to-blue-500 text-white py-2.5 rounded-lg hover:from-blue-500 hover:to-blue-400 transition font-semibold text-sm text-center"
            >
              일반 사용자로 로그인
            </button>
            <button 
              onClick={() => handleDevLogin('artist')}
              className="w-full bg-gradient-to-r from-purple-600 to-purple-500 text-white py-2.5 rounded-lg hover:from-purple-500 hover:to-purple-400 transition font-semibold text-sm text-center"
            >
              아티스트로 로그인
            </button>
            <button 
              onClick={() => handleDevLogin('venue')}
              className="w-full bg-gradient-to-r from-pink-600 to-pink-500 text-white py-2.5 rounded-lg hover:from-pink-500 hover:to-pink-400 transition font-semibold text-sm text-center"
            >
              공간 보유자로 로그인
            </button>
          </div>
        ...
        <DialogContent className="bg-[#1A1A1A] border-white/10 max-w-md">
          <DialogHeader>
            <DialogTitle className="text-white">
              {userType === 'artist' ? '아티스트 프로필' : '공간 프로필'}
            </DialogTitle>
          </DialogHeader>
          <div className="py-4 space-y-4">
            {userType === 'artist' ? (
